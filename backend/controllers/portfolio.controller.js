...
export const getPortfolioProject = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      message: "Fetching portfolio projects",
      portfolioProjects,
      friends,
      hackathons,
      user,
    ...
    return res
      .status(500)
      .json({ message: "Internal error while fetching portfolio projects" });
  }
};
...
export const submitPortfolioProject = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Portfolio project submitted successfully", project });
  } catch (error) {
    console.error("Internal error while submitting portfolio project", error);
    return res
      .status(500)
      .json({ message: "Internal error while submitting portfolio project" });
  }
};
...
export const getSpecificPortfolioProject = async (req, res) => {
  try {
    ...
    if (!portfolioProjectId) {
      return res
        .status(400)
        .json({ message: "Portfolio Projeect id is not defined" });
    }
    ...
    return res.status(200).json({
      message: "Fetching specific portfolio project",
      portfolioProject,
    });
  } catch (error) {
    console.log(
      "Internal error while fetching specific portfolio project",
      error
    );
    return res.status(500).json({
      message: "Internal error while fetching specific portfolio project",
    });
  }
};
